...
def find_resources():
    for res in NISysCfgHardwareEnumerator():
        try:
            attrs = ['VendorName', 'ProductName', 'SerialNumber', 'ConnectsToLinkName', 
                     'PxiPciBusNumber', 'PxiPciDeviceNumber', 'PxiPciFunctionNumber', 
                     'GpibPrimaryAddress', 'GpibSecondaryAddress', 
                     'ProvidesBusType', 'ProvidesLinkName']
            infos = dict()
            for attr in attrs:
                try:
                    ...
                    if e.status != NISysCfgStatus.PropDoesNotExist:
                        raise
...
def daqmx_devices():
    return [info['daqmx']['ExpertUserAlias'] 
            for info in find_resources()
            if 'daqmx' in info]
            
def scope_devices():
    return [info['scope']['ExpertUserAlias'] 
            for info in find_resources()
            if 'scope' in info]