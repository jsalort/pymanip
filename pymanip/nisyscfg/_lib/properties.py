...
from pymanip.nisyscfg._lib.constants import NISysCfgResourceProperty, NISysCfgIndexedProperty, NISYSCFG_SIMPLE_STRING_LENGTH, NISysCfgStatus
...
def NISysCfgGetResourceIndexedProperty(resourceHandle):
    ...
    if status != 0:
        raise RuntimeError(f'NISysCfgGetResourceProperty returned {str(NISysCfgStatus(status)):}')
    data = list()
    for index in range(num_experts):
        ...
        for attr in NISysCfgIndexedProperty:
            ...
            if status != 0:
                raise RuntimeError(f'NISysCfgGetResourceIndexedProperty returned {str(NISysCfgStatus(status)):}')
