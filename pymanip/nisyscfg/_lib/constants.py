...
class NISysCfgStatus(IntEnum):
    ...
    EndOfEnum = (
        1
    )  # Reached end of the enumeration. Used by the NISysCfgNext* functions.
    SelfTestBasicOnly = (
        0x00040370
    ...
    NotImplemented = (
        0x80004001
    )  # This operation is not implemented for this target or resource.
    ...
    PropMismatch = (
        0x80040370
    )  # The property already exists with a different type or value.
    ...
    ValueInvalid = (
        0x8004037F
    )  # A property contained a value that is not valid or is out of range.
    ValuesInconsistent = (
        0x80040380
    ...
    FirmwareImageDeviceMismatch = (
        0x80040386
    )  # The uploaded firmware image does not work with this resource.
    FirmwareImageCorrupt = (
        0x80040387
    )  # The uploaded firmware image is corrupt or incomplete.
    InvalidFirmwareVersion = (
        0x80040388
    )  # The specified firmware version does not exist.
    ...
    CannotDeletePresentResource = (
        0x8004038F
    )  # Could not delete the specified resource because it is present.
    ...
    PermissionDenied = (
        0x800403B0
    )  # Unable to write to file or folder. Permission denied.
    SystemNotFound = (
        0x800403B1
    ...
    InternalTimeout = (
        0x800403B5
    )  # Launched nimax.exe but it did not complete in a reasonable time.
    MissingTransform = (
        0x800403B6
    )  # Unable to find an XSL transform to generate the report.
    ...
    InsufficientPermissions = (
        0x80040401
    )  # The object cannot be accessed because of insufficient permissions.
    NCECopierFailed = 0x80040402  # Error with the object copier.
    FileOperationFailed = 0x80040403  # Error performing a file operation.
    NameCollisionError = (
        0x80040404
    )  # Names from one expert have collided with another expert.
    ...
    SystemNotSupported = (
        0x80040413
    )  # A specified system is not supported by this expert.
    SystemNotReachable = (
        0x80040414
    )  # A specified system is presumably supported, but network errors prevent connection.
    ProductSoftwareNotInstalled = (
        0x80040415
    )  # The product is not installed on the specified system.
    ProductSoftwareTooOld = (
        0x80040416
    )  # The product is installed on the remote system, but is too old.
    ProductSoftwareTooNew = (
        0x80040417
    )  # The product is installed on the remote system, but is too new.
    DataTooOld = (
        0x80040418
    ...
    NoItemsToCopy = (
        0x8004041A
    )  # The operation failed because no source items were specified.
    ...
    FileOverwrite = (
        0x8004041D
    )  # The operation failed because it would overwrite a file.
    ItemOverwrite = 0x8004041E  # The operation failed because it would overwrite items.
    MissingDependency = (
        0x8004041F
    )  # The operation failed because of missing dependency items.
    ...
    ImageInvalidCorrupt = (
        0x8004046D
    )  # The image is corrupt or the file type is invalid.
    ...
    ConfigFailed = (
        0x80040475
    )  # Remote system replied with the failure to config command.
    ...
    HDFormatNotSafeMode = (
        0x80040490
    )  # System must be in safe mode before attempting Hard Drive Format.
    ...
    GetRemoteFilesFailed = (
        0x80040495
    )  # Failed to get one or more files while creating system image.
    PutRemoteFilesFailed = (
        0x80040496
    )  # Failed to put one or more files while applying system image.
    ...
    OverwriteError = (
        0x800404A2
    )  # The file or directory exists and the overwrite flag was false.
    ...
    FileOrPathTooLong = (
        0x800404A4
    )  # Filename or pathname is longer than what the server supports.
    DDPInternalTimeout = (
        0x800404A5
    ...
    PathAlreadyExists = (
        0x800404A7
    )  # The operation failed because the path already exists.
    ExecutionFailure = (
        0x800404A8
    )  # The execution of an external command, script, or application failed.
    DownloadError = (
        0x800404A9
    )  # Failed to download the file from the 'RT Images' repository.
    ...
    NotUninstallable = (
        0x800404B6
    )  # Cannot uninstall a component. There are dependencies.
    DuplicatesNotAllowed = (
        0x800404B7
    )  # Cannot install multiple packages of the same component.
    ...
    CorruptConfig = (
        0x800404BD
    )  # Duplicate or missing components on target installation.
    ...
    TopLevelHiddenComponentError = (
        0x800404C3
    )  # There is a top-level hidden component installed.
    InvalidAddon = (
        0x800404C4
    ...
    NoRTImagesFolder = (
        0x800404C5
    )  # Could not find or access 'RT Images' repository location.
